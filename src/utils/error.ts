...
export const errorSchema = z.object({
  ...
  instance: z.string().optional().openapi({ description: "Request path", example: "/echo" }),
  errors: z
    .record(z.string())
    .optional()
    .openapi({ description: "Error details", example: { "path.to.error": "Error message" } }),
});
...
export class CustomError extends HTTPException {
  ...
  constructor({ type = "about:blank", detail, title, status, errors }: CustomErrorProps) {
    super(status);
...
export async function handleCustomErrorJSON(c: Context<BlankEnv, any, {}>, next: Next) {
  try {
    await next();
  } catch (error) {
